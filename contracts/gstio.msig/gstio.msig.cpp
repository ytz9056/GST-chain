#include <gstio.msig/gstio.msig.hpp>
#include <gstiolib/action.hpp>
#include <gstiolib/permission.hpp>
...
void multisig::propose() {
   ...
   gstio_assert( trx_header.expiration >= gstio::time_point_sec(now()), "transaction expired" );
   //gstio_assert( trx_header.actions.size() > 0, "transaction must have at least one action" );

   proposals proptable( _self, proposer );
   gstio_assert( proptable.find( proposal_name ) == proptable.end(), "proposal with the same name exists" );
   ...
   gstio_assert( res > 0, "transaction authorization failed" );
...
void multisig::approve( account_name proposer, name proposal_name, permission_level level ) {
   ...
   gstio_assert( itr != apps.requested_approvals.end(), "approval is not on the list of requested approvals" );
...
void multisig::unapprove( account_name proposer, name proposal_name, permission_level level ) {
   ...
   gstio_assert( itr != apps.provided_approvals.end(), "no approval previously granted" );
...
void multisig::cancel( account_name proposer, name proposal_name, account_name canceler ) {
   ...
   if( canceler != proposer ) {
      gstio_assert( unpack<transaction_header>( prop.packed_transaction ).expiration < gstio::time_point_sec(now()), "cannot cancel until expiration" );
   }
...
GSTIO_ABI( gstio::multisig, (propose)(approve)(unapprove)(cancel)(exec) )
