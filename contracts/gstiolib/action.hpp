...
namespace gstio {
   ...
   struct action {
      ...
      void send_context_free() const {
         gstio_assert( authorization.size() == 0, "context free actions cannot have authorizations");
         auto serialize = pack(*this);
      ...
      template<typename T>
      T data_as() {
         gstio_assert( name == T::get_name(), "Invalid name" );
         gstio_assert( account == T::get_account(), "Invalid account" );
...
INLINE_ACTION_SENDER3( CONTRACT_CLASS, NAME, ::gstio::string_to_name(#NAME) )
 ...
 * @param NAME - The name of the action
 */
#define ACTION( CODE, NAME ) struct NAME : ::gstio::action_meta<CODE, ::gstio::string_to_name(#NAME) >
