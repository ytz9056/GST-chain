...
#include <gstiolib/system.h>
#include <gstiolib/symbol.hpp>
#include <tuple>
#include <limits>
...
namespace gstio {
   ...
   */
   ...
   struct asset {
      ...
      :amount(a),symbol{s}
      {
         gstio_assert( is_amount_within_range(), "magnitude of asset amount must be less than 2^62" );
         gstio_assert( symbol.is_valid(),        "invalid symbol name" );
      }
      ...
      asset& operator-=( const asset& a ) {
         gstio_assert( a.symbol == symbol, "attempt to subtract asset with different symbol" );
         amount -= a.amount;
         gstio_assert( -max_amount <= amount, "subtraction underflow" );
         gstio_assert( amount <= max_amount,  "subtraction overflow" );
         return *this;
      }
       ...
       */
      asset& operator+=( const asset& a ) {
         gstio_assert( a.symbol == symbol, "attempt to add asset with different symbol" );
         amount += a.amount;
         gstio_assert( -max_amount <= amount, "addition underflow" );
         gstio_assert( amount <= max_amount,  "addition overflow" );
         return *this;
      }
      ...
      asset& operator*=( int64_t a ) {
         ...
         gstio_assert( tmp <= max_amount, "multiplication overflow" );
         gstio_assert( tmp >= -max_amount, "multiplication underflow" );
         amount = (int64_t)tmp;
         return *this;
      }
       ...
       */
      asset& operator/=( int64_t a ) {
         gstio_assert( a != 0, "divide by zero" );
         gstio_assert( !(amount == std::numeric_limits<int64_t>::min() && a == -1), "signed division overflow" );
         amount /= a;
         return *this;
      }
      ...
      friend int64_t operator/( const asset& a, const asset& b ) {
         gstio_assert( a.symbol == b.symbol, "comparison of assets with different symbols is not allowed" );
   ...
   */
   struct extended_asset : public asset {
      ...
      friend extended_asset operator - ( const extended_asset& a, const extended_asset& b ) {
         gstio_assert( a.contract == b.contract, "type mismatch" );
      ...
      GSTLIB_SERIALIZE( extended_asset, (amount)(symbol)(contract) )
   };
