...
/**
 *  The purpose of this contract is to implement something like Steem on GST, this
...
void apply_social_post() {
   ...
   gstio_assert( current_context() == post.author, "cannot call from any other context" );
...
void apply_social_vote() {
   ...
   if( context == vote.author ) {
      ...
      gstio_assert( Db::get( vote.postid, post ) > 0, "unable to find post" );
      gstio_assert( now() - post.created < days(7), "cannot vote after 7 days" );
      ...
      gstio_assert( abs_vote <= vote_account.vote_power, "insufficient vote power" );
      post.votes += vote.vote_power;
      vote_account.vote_power -= abs_vote;
      vote_account.last_vote  = now();
      Db::store( "account", vote_account );
   } else {
      gstio_assert( false, "invalid context for execution of this vote" );
   }
}
