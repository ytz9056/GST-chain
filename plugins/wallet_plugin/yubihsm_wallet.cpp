...
#include <gstio/wallet_plugin/yubihsm_wallet.hpp>
#include <gstio/chain/exceptions.hpp>
#include <gstio/wallet_plugin/yubihsm.h>
...
struct yubihsm_wallet_impl {
   ...
   public_key_type create() {
      ...
      try {
         if((rc = api.util_generate_key_ec(session, &new_key_id, "kgstd created key", authkey_domains, &creation_caps, YH_ALGO_EC_P256)))
