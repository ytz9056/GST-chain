...
class producer_plugin_impl : public std::enable_shared_from_this<producer_plugin_impl> {
   public:
      ...
      uint32_t _production_skip_flags              = 0; //gstio::chain::skip_nothing;
...
void new_chain_banner(const gstio::chain::controller& db)
{
   std::cerr << "\n"
      "*******************************\n"
      "*                             *\n"
...
chain::signature_type producer_plugin::sign_compact(const chain::public_key_type& key, const fc::sha256& digest) const
{
  if(key != chain::public_key_type()) {
    auto private_key_itr = my->_signature_providers.find(key);
    GST_ASSERT(private_key_itr != my->_signature_providers.end(), producer_priv_key_not_found, "Local producer has no private key in config.ini corresponding to public key ${key}", ("key", key));
...
static producer_plugin_impl::signature_provider_type
make_kgstd_signature_provider(const std::shared_ptr<producer_plugin_impl>& impl, const string& url_str, const public_key_type pubkey) {
   fc::url kgstd_url;
   if(boost::algorithm::starts_with(url_str, "unix://"))
      ...
      kgstd_url = fc::url("unix", url_str.substr(7), ostring(), ostring(), ostring(), ostring(), ovariant_object(), fc::optional<uint16_t>());
   else
      kgstd_url = fc::url(url_str);
   std::weak_ptr<producer_plugin_impl> weak_impl = impl;

   return [weak_impl, kgstd_url, pubkey]( const chain::digest_type& digest ) {
      auto impl = weak_impl.lock();
      if (impl) {
         fc::variant params;
         fc::to_variant(std::make_pair(digest, pubkey), params);
         auto deadline = impl->_kgstd_provider_timeout_us.count() >= 0 ? fc::time_point::now() + impl->_kgstd_provider_timeout_us : fc::time_point::maximum();
         return app().get_plugin<http_client_plugin>().get_client().post_sync(kgstd_url, params, deadline).as<chain::signature_type>();
      } else {
...
{ try {
   ...
   if( options.count("signature-provider") ) {
      ...
      for (const auto& key_spec_pair : key_spec_pairs) {
         try {
            ...
            GST_ASSERT(delim != std::string::npos, plugin_config_exception, "Missing \"=\" in the key spec pair");
            auto pub_key_str = key_spec_pair.substr(0, delim);
            auto spec_str = key_spec_pair.substr(delim + 1);

            auto spec_delim = spec_str.find(":");
            GST_ASSERT(spec_delim != std::string::npos, plugin_config_exception, "Missing \":\" in the key spec pair");
            ...
            if (spec_type_str == "KEY") {
               ...
               my->_signature_providers[pubkey] = make_kgstd_signature_provider(my, spec_data, pubkey);
            }
   ...
   my->_kgstd_provider_timeout_us = fc::milliseconds(options.at("kgstd-provider-timeout").as<int32_t>());
   ...
   if( options.count( "snapshots-dir" )) {
      ...
      GST_ASSERT( fc::is_directory(my->_snapshots_dir), snapshot_directory_not_found_exception,
...
{ try {
   ...
   GST_ASSERT( my->_producers.empty() || chain.get_validation_mode() == chain::validation_mode::FULL, plugin_config_exception,
   ...
   if (!my->_producers.empty()) {
      ...
      if (my->_production_enabled) {
         ...
         //_production_skip_flags |= gstio::chain::skip_undo_history_check;
      }
   }
...
producer_plugin::snapshot_information producer_plugin::create_snapshot() const {
   ...
   GST_ASSERT( !fc::is_regular_file(snapshot_path), snapshot_exists_exception,
...
void producer_plugin_impl::produce_block() {
   ...
   GST_ASSERT(signature_provider_itr != _signature_providers.end(), producer_priv_key_not_found, "Attempting to produce a block for which we don't have the private key");
