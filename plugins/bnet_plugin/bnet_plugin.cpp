...
#include <gstio/bnet_plugin/bnet_plugin.hpp>
...
using gstio::public_key_type;
using gstio::chain_id_type;
using gstio::block_id_type;
using gstio::block_timestamp_type;
using std::string;
using gstio::sha256;
using gstio::signed_block_ptr;
using gstio::packed_transaction_ptr;
using std::vector;
...
namespace gstio {
       ...
       //    shared_ptr< vector<char> >  block_msg; ///< packed bnet_message for this block
        ...
        void on( const signed_block_ptr& b ) {
           ...
           if (!b) {
              ...
              GST_THROW(block_validate_exception, "bad block" );
           }
  ...
  class listener : public std::enable_shared_from_this<listener> {
     ...
     public:
        ...
        void run() {
           GST_ASSERT( _acceptor.is_open(), plugin_exception, "unable top open listen socket" );
           do_accept();
        }
   ...
   void bnet_plugin::plugin_shutdown() {
      ...
      my->for_each_session([](auto ses){
         GST_ASSERT( false, plugin_exception, "session ${ses} still active", ("ses", ses->_session_num) );
      });
