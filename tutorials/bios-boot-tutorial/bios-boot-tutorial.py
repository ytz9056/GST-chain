...
fastUnstakeSystem = './fast.refund/gstio.system/gstio.system.wasm'
...
def startWallet():
    ...
    background(args.kgstd + ' --unlock-timeout %d --http-server-address 127.0.0.1:6666 --wallet-dir %s' % (unlockTimeout, os.path.abspath(args.wallet_dir)))
    sleep(.4)
    run(args.clgst + 'wallet create --to-console')

def importKeys():
    run(args.clgst + 'wallet import --private-key ' + args.private_key)
    keys = {}
    for a in accounts:
        key = a['pvt']
        if not key in keys:
            if len(keys) >= args.max_user_keys:
                break
            keys[key] = True
            run(args.clgst + 'wallet import --private-key ' + key)
...
def createSystemAccounts():
    for a in systemAccounts:
        run(args.clgst + 'create account gstio ' + a + ' ' + args.public_key)
...
def createStakedAccounts(b, e):
    ...
    for i in range(b, e):
        ...
        retry(args.clgst + 'system newaccount --transfer gstio %s %s --stake-net "%s" --stake-cpu "%s" --buy-ram "%s"   ' % 
            (a['name'], a['pub'], intToCurrency(stakeNet), intToCurrency(stakeCpu), intToCurrency(ramFunds)))
        if unstaked:
            retry(args.clgst + 'transfer gstio %s "%s"' % (a['name'], intToCurrency(unstaked)))

def regProducers(b, e):
    for i in range(b, e):
        a = accounts[i]
        retry(args.clgst + 'system regproducer ' + a['name'] + ' ' + a['pub'] + ' https://' + a['name'] + '.com' + '/' + a['pub'])

def listProducers():
    run(args.clgst + 'system listproducers')
...
def claimRewards():
    table = getJsonOutput(args.clgst + 'get table gstio gstio producers -l 100')
    times = []
    for row in table['rows']:
        if row['unpaid_blocks'] and not row['last_claim_time']:
            times.append(getJsonOutput(args.clgst + 'system claimrewards -j ' + row['owner'])['processed']['elapsed'])
    print('Elapsed time for claimrewards:', times)

def proxyVotes(b, e):
    vote(firstProducer, firstProducer + 1)
    proxy = accounts[firstProducer]['name']
    retry(args.clgst + 'system regproxy ' + proxy)
    sleep(1.0)
    for i in range(b, e):
        voter = accounts[i]['name']
        retry(args.clgst + 'system voteproducer proxy ' + voter + ' ' + proxy)

def updateAuth(account, permission, parent, controller):
    run(args.clgst + 'push action gstio updateauth' + jsonArg({
        'account': account,
...
def resign(account, controller):
    ...
    sleep(1)
    run(args.clgst + 'get account ' + account)
...
def randomTransfer(b, e):
    for j in range(20):
        ...
        run(args.clgst + 'transfer -f ' + src + ' ' + dest + ' "0.0001 ' + args.symbol + '"' + ' || true')
...
def msigProposeReplaceSystem(proposer, proposalName):
    ...
    trxPermissions = [{'actor': 'gstio', 'permission': 'active'}]
    with open(fastUnstakeSystem, mode='rb') as f:
        setcode = {'account': 'gstio', 'vmtype': 0, 'vmversion': 0, 'code': f.read().hex()}
    run(args.clgst + 'multisig propose ' + proposalName + jsonArg(requestedPermissions) + 
        jsonArg(trxPermissions) + 'gstio setcode' + jsonArg(setcode) + ' -p ' + proposer)

def msigApproveReplaceSystem(proposer, proposalName):
    for i in range(firstProducer, firstProducer + numProducers):
        run(args.clgst + 'multisig approve ' + proposer + ' ' + proposalName +
            jsonArg({'actor': accounts[i]['name'], 'permission': 'active'}) +
            '-p ' + accounts[i]['name'])

def msigExecReplaceSystem(proposer, proposalName):
    retry(args.clgst + 'multisig exec ' + proposer + ' ' + proposalName + ' -p ' + proposer)

def msigReplaceSystem():
    run(args.clgst + 'push action gstio buyrambytes' + jsonArg(['gstio', accounts[0]['name'], 200000]) + '-p gstio')
    sleep(1)
...
def produceNewAccounts():
    with open('newusers', 'w') as f:
        for i in range(120_000, 200_000):
            x = getOutput(args.clgst + 'create key --to-console')
...
def stepLog():
    run('tail -n 60 ' + args.nodes_dir + '00-gstio/stderr')
...
parser.add_argument('--private-Key', metavar='', help="GSTIO Private Key", default='5K463ynhZoCDDa4RDcr63cUwWLTnKqmdcoTKTHBjqoKfv4u5V7p', dest="private_key")
parser.add_argument('--clgst', metavar='', help="Clgst command", default='../../build/programs/clgst/clgst --wallet-url http://127.0.0.1:6666 ')
parser.add_argument('--nodgst', metavar='', help="Path to nodgst binary", default='../../build/programs/nodgst/nodgst')
parser.add_argument('--kgstd', metavar='', help="Path to kgstd binary", default='../../build/programs/kgstd/kgstd')
parser.add_argument('--contracts-dir', metavar='', help="Path to contracts directory", default='../../build/contracts/')
parser.add_argument('--nodes-dir', metavar='', help="Path to nodes directory", default='./nodes/')
parser.add_argument('--genesis', metavar='', help="Path to genesis.json", default="./genesis.json")
parser.add_argument('--wallet-dir', metavar='', help="Path to wallet directory", default='./wallet/')
parser.add_argument('--log-path', metavar='', help="Path to log file", default='./output.log')
parser.add_argument('--symbol', metavar='', help="The gstio.system symbol", default='SYS')
...
parser.add_argument('-H', '--http-port', type=int, default=8000, metavar='', help='HTTP port for clgst')
...
args.clgst += '--url http://127.0.0.1:%d ' % args.http_port
