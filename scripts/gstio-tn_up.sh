#!/bin/bash
#
# gstio-tn_up is a helper script used to start a node that was previously stopped.
# It is not intended to be run stand-alone; it is a companion to the
# gstio-tn_bounce.sh and gstio-tn_roll.sh scripts.

connected="0"

rundir=programs/nodgst
prog=nodgst
...
if [ ! -e $rundir/$prog ]; then
    echo unable to locate binary for nodgst
    exit -1
fi

if [ -z "$GSTIO_NODE" ]; then
    echo data directory not set
    exit -1
fi

datadir=var/lib/node_$GSTIO_NODE
...
relaunch() {
    echo "$rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/gstio/node_$GSTIO_NODE > $datadir/stdout.txt  2>> $datadir/$log "
    nohup $rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/gstio/node_$GSTIO_NODE > $datadir/stdout.txt  2>> $datadir/$log &
    pid=$!
    echo pid = $pid
