...
namespace gstio { namespace chain { namespace config {
...
const static uint64_t system_account_name    = N(gstio);
...
const static uint64_t gstio_auth_scope       = N(gstio.auth);
const static uint64_t gstio_all_scope        = N(gstio.all);

const static uint64_t active_name = N(active);
const static uint64_t owner_name  = N(owner);
const static uint64_t gstio_any_name = N(gstio.any);
const static uint64_t gstio_code_name = N(gstio.code);
...
const static gstio::chain::wasm_interface::vm_type default_wasm_runtime = gstio::chain::wasm_interface::vm_type::wabt;
...
} } } // namespace gstio::chain::config

constexpr uint64_t GST_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / gstio::chain::config::percent_100;
}

template<typename Number>
Number GST_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + gstio::chain::config::percent_100 - gstio::chain::config::percent_1)  / gstio::chain::config::percent_100;
}
