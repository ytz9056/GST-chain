...
namespace impl {
   ...
   struct abi_from_variant {
      ...
      static void extract( const variant& v, action& act, Resolver resolver, abi_traverse_context& ctx )
      {
         auto h = ctx.enter_scope();
         const variant_object& vo = v.get_object();
         GST_ASSERT(vo.contains("account"), packed_transaction_type_exception, "Missing account");
         GST_ASSERT(vo.contains("name"), packed_transaction_type_exception, "Missing name");
         ...
         GST_ASSERT(valid_empty_data || !act.data.empty(), packed_transaction_type_exception,
      ...
      static void extract( const variant& v, packed_transaction& ptrx, Resolver resolver, abi_traverse_context& ctx )
      {
         auto h = ctx.enter_scope();
         const variant_object& vo = v.get_object();
         GST_ASSERT(vo.contains("signatures"), packed_transaction_type_exception, "Missing signatures");
         GST_ASSERT(vo.contains("compression"), packed_transaction_type_exception, "Missing compression");
         ...
         if( vo.contains("packed_trx") && vo["packed_trx"].is_string() && !vo["packed_trx"].as_string().empty() ) {
            ...
            GST_ASSERT(vo.contains("transaction"), packed_transaction_type_exception, "Missing transaction");
            transaction trx;
            vector<bytes> context_free_data;
