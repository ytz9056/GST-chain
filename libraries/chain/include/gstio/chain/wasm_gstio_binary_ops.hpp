...
namespace gstio { namespace chain { namespace wasm_ops {
...
struct field_specific_params {
   ...
   static void pack(instruction_stream* stream, Field& f) { return gstio::chain::wasm_ops::pack(stream, f); }
   static auto to_string(Field& f) { return std::string(" ")+
                                       gstio::chain::wasm_ops::to_string(f); }
};
template <>
...
template <class Op_Types>
struct GSTIO_OperatorDecoderStream
{
   GSTIO_OperatorDecoderStream(const std::vector<U8>& codeBytes)
   ...
   instr* decodeOp() {
      GST_ASSERT(nextByte + sizeof(IR::Opcode) <= end, wasm_exception, "");
      ...
      #define VISIT_OPCODE(opcode,name,nameString,Imm,...) \
         case IR::Opcode::name: \
         { \
            GST_ASSERT(nextByte + sizeof(IR::OpcodeAndImm<IR::Imm>) <= end, wasm_exception, ""); \
...
template <class Op_Types>
const std::vector<instr*>* GSTIO_OperatorDecoderStream<Op_Types>::_cached_ops;

}}} // namespace gstio, chain, wasm_ops

FC_REFLECT_TEMPLATE( (typename T), gstio::chain::wasm_ops::block< T >, (code)(rt) )
FC_REFLECT( gstio::chain::wasm_ops::memarg, (a)(o) )
FC_REFLECT( gstio::chain::wasm_ops::blocktype, (result) )
FC_REFLECT( gstio::chain::wasm_ops::memoryoptype, (end) )
