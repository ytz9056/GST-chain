...
#define N(X) gstio::chain::string_to_name(#X)
...
namespace std {
   template<> struct hash<gstio::chain::name> : private hash<uint64_t> {
      typedef gstio::chain::name argument_type;
...
namespace fc {
  class variant;
  void to_variant(const gstio::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, gstio::chain::name& check);
} // fc


FC_REFLECT( gstio::chain::name, (value) )
