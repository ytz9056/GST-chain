#pragma once
#include <gstio/chain/block_header.hpp>
...
FC_REFLECT_ENUM( gstio::chain::transaction_receipt::status_enum,
                 (executed)(soft_fail)(hard_fail)(delayed)(expired) )

FC_REFLECT(gstio::chain::transaction_receipt_header, (status)(cpu_usage_us)(net_usage_words) )
FC_REFLECT_DERIVED(gstio::chain::transaction_receipt, (gstio::chain::transaction_receipt_header), (trx) )
FC_REFLECT_DERIVED(gstio::chain::signed_block, (gstio::chain::signed_block_header), (transactions)(block_extensions) )
