...
namespace gstio { namespace chain { namespace wasm_validations {
   ...
   struct nested_validator {
      ...
      static void accept( wasm_ops::instr* inst, wasm_ops::visitor_arg& arg ) {
         if (!disabled) {
            ...
            depth++;
            GST_ASSERT(depth < 1024, wasm_execution_error, "Nested depth exceeded");
         }
      }
   };
   ...
   class wasm_binary_validation {
      ...
      public:
         wasm_binary_validation( const gstio::chain::controller& control, IR::Module& mod ) : _module( &mod ) {
         ...
         void validate() {
            ...
            for ( auto& fd : _module->functions.defs ) {
               wasm_ops::GSTIO_OperatorDecoderStream<op_constrainers> decoder(fd.code);
               while ( decoder ) {
