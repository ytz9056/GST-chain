...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        set_privileged(N(gstio.token));

        // Verify gstio.msig and gstio.token is privileged
        const auto& gstio_msig_acc = get<account_object, by_name>(N(gstio.msig));
        BOOST_TEST(gstio_msig_acc.privileged == true);
        const auto& gstio_token_acc = get<account_object, by_name>(N(gstio.token));
        BOOST_TEST(gstio_token_acc.privileged == true);


        // Create SYS tokens in gstio.token, set its manager as gstio
        ...
        create_currency(N(gstio.token), config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion SYS tokens to gstio.system
        issue(N(gstio.token), config::system_account_name, config::system_account_name, initial_supply);
        ...
        for( const auto& a : test_genesis ) {
           ...
           r = delegate_bandwidth(N(gstio.stake), a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        BOOST_REQUIRE_THROW(claim_rewards(N(runnerup1)), gstio_assert_message_exception);
        ...
        BOOST_REQUIRE_THROW(undelegate_bandwidth(N(b1), N(b1), core_from_string("49999500.0000"), core_from_string("49999500.0000")), gstio_assert_message_exception);
