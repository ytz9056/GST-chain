...
namespace gstio_system {

class gstio_system_tester : public TESTER {
public:

   gstio_system_tester()
   : gstio_system_tester([](TESTER& ) {}){}

   template<typename Lambda>
   gstio_system_tester(Lambda setup) {
      setup(*this);
   ...
   action_result buyram( const account_name& payer, account_name receiver, const asset& gstin ) {
      return push_action( payer, N(buyram), mvo()( "payer",payer)("receiver",receiver)("quant",gstin) );
   }
   ...
   asset get_balance( const account_name& act ) {
      vector<char> data = get_row_by_account( N(gstio.token), act, N(accounts), symbol(CORE_SYMBOL).to_symbol_code().value );
   ...
   abi_serializer initialize_multisig() {
      ...
      {
         create_account_with_resources( N(gstio.msig), config::system_account_name );
         BOOST_REQUIRE_EQUAL( success(), buyram( "gstio", "gstio.msig", core_from_string("5000.0000") ) );
         produce_block();
   ...
   vector<name> active_and_vote_producers() {
      ...
      transfer( "gstio", "alice1111111", core_from_string("650000000.0000"), "gstio" );
      ...
      auto trace_auth = TESTER::push_action(config::system_account_name, updateauth::get_name(), config::system_account_name, mvo()
                                            ...
                                            ("auth",  authority(1, {key_weight{get_public_key( config::system_account_name, "active" ), 1}}, {
                                                  permission_level_weight{{config::system_account_name, config::gstio_code_name}, 1},
...
inline uint64_t M( const string& gst_str ) {
   return core_from_string( gst_str ).get_amount();
}
