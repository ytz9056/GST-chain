...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   // Link authority for gstio reqauth action with alice's spending key
   chain.link_authority("alice", "gstio", "spending",  "reqauth");
   ...
   BOOST_CHECK_THROW( chain.link_authority("alice", "gstio", "spending",  "reqauth"), action_validate_exception);

   // Unlink alice with gstio reqauth
   chain.unlink_authority("alice", "gstio", "reqauth");
   ...
   // Link authority for any gstio action with alice's scud key
   chain.link_authority("alice", "gstio", "scud");
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority("alice", "gstio", "first",  "reqauth");
...
try {
   ...
   // Creating account with gstio. prefix with privileged account
   chain.create_account("gstio.test1");

   // Creating account with gstio. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account("gstio.test2", "joe"), action_validate_exception,
                         fc_exception_message_is("only privileged accounts can have names that start with 'gstio.'"));
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( "alice", "gstio", "gstio.any", "reqauth" );
   chain.link_authority( "bob", "gstio", "gstio.any", "reqauth" );

   /// this should succeed because gstio::reqauth is linked to any permission
   chain.push_reqauth("alice", { permission_level{N(alice), "spending"} }, { spending_priv_key });
...
try {
   ...
   using resource_usage_object = gstio::chain::resource_limits::resource_usage_object;
   using by_owner = gstio::chain::resource_limits::by_owner;
...
BOOST_AUTO_TEST_CASE( linkauth_special ) { try {
   ...
   auto validate_disallow = [&] (const char *type) {
      BOOST_REQUIRE_EXCEPTION(
         ...
         fc_exception_message_is(std::string("Cannot link gstio::") + std::string(type) + std::string(" to a minimum permission"))
      );
   };
