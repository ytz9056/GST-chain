...
BOOST_AUTO_TEST_CASE( forking ) try {
   ...
   auto r2 = c.create_accounts( {N(gstio.token)} );
   wdump((fc::json::to_pretty_string(r2)));
   c.set_code( N(gstio.token), gstio_token_wast );
   c.set_abi( N(gstio.token), gstio_token_abi );
   c.produce_blocks(10);


   auto cr = c.push_action( N(gstio.token), N(create), N(gstio.token), mutable_variant_object()
              ("issuer",       "gstio" )
   ...
   cr = c.push_action( N(gstio.token), N(issue), config::system_account_name, mutable_variant_object()
              ("to",       "dan" )
