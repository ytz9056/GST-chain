...
class gstio_msig_tester : public tester {
public:

   gstio_msig_tester() {
      create_accounts( { N(gstio.msig), N(gstio.stake), N(gstio.ram), N(gstio.ramfee), N(alice), N(bob), N(carol) } );
      produce_block();
   ...
   void issue( name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(gstio.token), N(issue), manager, mutable_variant_object()
                                ("to",      to )
                                ("quantity", amount )
                                ("memo", "")
                                );
   ...
   void transfer( name from, name to, const string& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(gstio.token), N(transfer), manager, mutable_variant_object()
                                ("from",    from)
                                ("to",      to )
   ...
   asset get_balance( const account_name& act ) {
      ...
      const auto* tbl = db.find<table_id_object, by_code_scope_table>(boost::make_tuple(N(gstio.token), act, N(accounts)));
      share_type result = 0;
   ...
   transaction_trace_ptr push_action( const account_name& signer, const action_name& name, const variant_object& data, bool auth = true ) {
      ...
      auto trace = base_tester::push_action( N(gstio.msig), name, accounts, data );
      produce_block();
...
transaction gstio_msig_tester::reqauth( account_name from, const vector<permission_level>& auths, const fc::microseconds& max_serialization_time ) {
   fc::variants v;
...
BOOST_AUTO_TEST_SUITE(gstio_msig_tests)

BOOST_FIXTURE_TEST_CASE( propose_approve_execute, gstio_msig_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( propose_approve_unapprove, gstio_msig_tester ) try {
   auto trx = reqauth("alice", {permission_level{N(alice), config::active_name}}, abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( propose_approve_by_two, gstio_msig_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( propose_with_wrong_requested_auth, gstio_msig_tester ) try {
   auto trx = reqauth("alice", vector<permission_level>{ { N(alice), config::active_name },  { N(bob), config::active_name } }, abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( big_transaction, gstio_msig_tester ) try {
   vector<permission_level> perm = { { N(alice), config::active_name }, { N(bob), config::active_name } };
   auto wasm = wast_to_wasm( gstio_token_wast );
...
BOOST_FIXTURE_TEST_CASE( update_system_contract_all_approve, gstio_msig_tester ) try {

   // required to set up the link between (gstio active) and (gstio.prods active)
   //
   //                  gstio active
   //                       |
   //             gstio.prods active (2/3 threshold)
   ...
   set_authority(config::system_account_name, "active", authority(1,
      vector<key_weight>{{get_private_key("gstio", "active").get_public_key(), 1}},
   ...
   create_accounts( { N(gstio.token) } );
   set_code( N(gstio.token), gstio_token_wast );
   set_abi( N(gstio.token), gstio_token_abi );

   create_currency( N(gstio.token), config::system_account_name, core_from_string("10000000000.0000") );
   issue(config::system_account_name, core_from_string("1000000000.0000"));
   BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"),
                        get_balance("gstio") + get_balance("gstio.ramfee") + get_balance("gstio.stake") + get_balance("gstio.ram") );
   ...
   // execute by alice to replace the gstio system contract
   transaction_trace_ptr trace;
   control->applied_transaction.connect([&]( const transaction_trace_ptr& t) { if (t->scheduled) { trace = t; } } );
   ...
   BOOST_REQUIRE_EXCEPTION( create_account_with_resources( N(alice1111112), config::system_account_name, core_from_string("1.0000"), false ),
                            gstio_assert_message_exception, gstio_assert_message_is("Unknown Test")

   );
} FC_LOG_AND_RETHROW()

BOOST_FIXTURE_TEST_CASE( update_system_contract_major_approve, gstio_msig_tester ) try {

   // set up the link between (gstio active) and (gstio.prods active)
   set_authority(config::system_account_name, "active", authority(1,
      vector<key_weight>{{get_private_key("gstio", "active").get_public_key(), 1}},
      vector<permission_level_weight>{{{config::producers_account_name, config::active_name}, 1}}), "owner",
      { { config::system_account_name, "active" } }, { get_private_key( config::system_account_name, "active" ) });
   ...
   BOOST_REQUIRE_EXCEPTION(
      ...
      gstio_assert_message_exception, gstio_assert_message_is("transaction authorization failed")
   );
