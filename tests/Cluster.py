...
class Cluster(object):
    ...
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899, enableMongo=False
                 , mongoHost="localhost", mongoPort=27017, mongoDb="GSTtest", defproduceraPrvtKey=None, defproducerbPrvtKey=None, staging=False):
        """Cluster container.
        walletd [True|False] Is wallet kgstd running. If not load the wallet plugin
        localCluster [True|False] Is cluster local to host.
        host: gst server host
        port: gst server port
        walletHost: gst wallet host
        walletPort: wos wallet port
        enableMongo: Include mongoDb support, configures gst mongo plugin
        ...
        self.gstioAccount=self.defProducerAccounts["gstio"]= Account("gstio")
    ...
    def launch(self, pnodes=1, totalNodes=1, prodCount=1, topo="mesh", p2pPlugin="net", delay=1, onlyBios=False, dontBootstrap=False,
        ...
        extraNodgstArgs: string of arguments to pass through to each nodoes instance (via --nodgst flag on launcher)
        ...
        specificExtraNodgstArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-nodgst flags on launcher), example: { "5" : "--plugin gstio::test_control_api_plugin" }
        """
        ...
        while not Utils.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on nodgst default port. wait...")
            if tries == 0:
                return False
            tries = tries - 1
            time.sleep(2)
        ...
        nodgstArgs="--max-transaction-time -1 --abi-serializer-max-time-ms 990000 --filter-on * --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
            nodgstArgs += " --plugin gstio::wallet_api_plugin"
        if self.enableMongo:
            nodgstArgs += " --plugin gstio::mongo_db_plugin --mongodb-wipe --delete-all-blocks --mongodb-uri %s" % self.mongoUri
        if extraNodgstArgs is not None:
            assert(isinstance(extraNodgstArgs, str))
            nodgstArgs += extraNodgstArgs
        if Utils.Debug:
            nodgstArgs += " --contracts-console"

        if nodgstArgs:
            cmdArr.append("--nodgst")
            cmdArr.append(nodgstArgs)
        ...
        self.gstioAccount=self.defProducerAccounts["gstio"]
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        fromm=source
        to=accounts[0]
        Utils.Print("Transfer %s units from account %s to %s on gst server port %d" % (
    ...
    def validateAccounts(self, accounts, testSysAccounts=True):
        ...
        myAccounts = []
        if testSysAccounts:
            myAccounts += [self.gstioAccount, self.defproduceraAccount, self.defproducerbAccount]
        if accounts:
            assert(isinstance(accounts, list))
            myAccounts += accounts
    ...
    def parseClusterKeys(totalNodes):
        ...
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse gstio private keys from cluster config files.")
            return None
    ...
    def bios_bootstrap(totalNodes, biosHost, biosPort, walletMgr, silent=False):
        """Bootstrap cluster using the bios_boot.sh script generated by gstio-launcher."""
        ...
        gstioName="gstio"
        gstioKeys=producerKeys[gstioName]
        gstioAccount=Account(gstioName)
        gstioAccount.ownerPrivateKey=gstioKeys["private"]
        gstioAccount.ownerPublicKey=gstioKeys["public"]
        gstioAccount.activePrivateKey=gstioKeys["private"]
        gstioAccount.activePublicKey=gstioKeys["public"]
        producerKeys.pop(gstioName)

        if not walletMgr.importKey(gstioAccount, ignWallet):
            Utils.Print("ERROR: Failed to import %s account keys into ignition wallet." % (gstioName))
            return None
        ...
        for name, keys in producerKeys.items():
            ...
            if name != "gstio":
                ...
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from gstio.token to %s." % (name))
                    return None
    ...
    def bootstrap(totalNodes, prodCount, totalProducers, biosHost, biosPort, walletMgr, onlyBios=False):
        ...
        trans=biosNode.publishContract(gstioAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
        ...
        for name, keys in producerKeys.items():
            ...
            trans=biosNode.createAccount(initx, gstioAccount, 0)
            if trans is None:
        ...
        if not onlyBios:
            if prodCount == -1:
                ...
                with open(setProdsFile, "r") as f:
                    ...
                    opts="--permission gstio@active"
                    myTrans=biosNode.pushMessage("gstio", "setprods", setProdsStr, opts)
                ...
                trans=biosNode.pushMessage("gstio", "setprods", setProdsStr, opts)
                if trans is None or not trans[0]:
            ...
            # wait for block production handover (essentially a block produced by anyone but gstio).
            lam = lambda: biosNode.getInfo(exitOnError=True)["head_block_producer"] != "gstio"
            ret=Utils.waitForBool(lam)
            if not ret:
                Utils.Print("ERROR: Block production handover failed.")
                return None

        gstioTokenAccount=copy.deepcopy(gstioAccount)
        gstioTokenAccount.name="gstio.token"
        trans=biosNode.createAccount(gstioTokenAccount, gstioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (gstioTokenAccount.name))
            return None

        gstioRamAccount=copy.deepcopy(gstioAccount)
        gstioRamAccount.name="gstio.ram"
        trans=biosNode.createAccount(gstioRamAccount, gstioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (gstioRamAccount.name))
            return None

        gstioRamfeeAccount=copy.deepcopy(gstioAccount)
        gstioRamfeeAccount.name="gstio.ramfee"
        trans=biosNode.createAccount(gstioRamfeeAccount, gstioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (gstioRamfeeAccount.name))
            return None

        gstioStakeAccount=copy.deepcopy(gstioAccount)
        gstioStakeAccount.name="gstio.stake"
        trans=biosNode.createAccount(gstioStakeAccount, gstioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (gstioStakeAccount.name))
            return None
        ...
        abiFile="%s.abi" % (contract)
        Utils.Print("Publish %s contract" % (contract))
        trans=biosNode.publishContract(gstioTokenAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
            Utils.Print("ERROR: Failed to publish contract %s." % (contract))
            return None

        # Create currency0000, followed by issue currency0000
        contract=gstioTokenAccount.name
        Utils.Print("push create action to %s contract" % (contract))
        action="create"
        data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\",\"can_freeze\":\"0\",\"can_recall\":\"0\",\"can_whitelist\":\"0\"}" % (gstioTokenAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (contract)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push create action to gstio contract.")
            return None
        ...
        action="issue"
        data="{\"to\":\"%s\",\"quantity\":\"1000000000.0000 %s\",\"memo\":\"initial issue\"}" % (gstioAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (contract)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push issue action to gstio contract.")
            return None
        ...
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"%s\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (gstioAccount.name, name, initialFunds, "init transfer")
            opts="--permission %s@active" % (gstioAccount.name)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to transfer funds from %s to %s." % (gstioTokenAccount.name, name))
                return None
    ...
    def bounce(self, nodes, silent=True):
        """Bounces nodgst instances as indicated by parameter nodes.
    ...
    def down(self, nodes, silent=True):
        """Brings down nodgst instances as indicated by parameter nodes.
        nodes should take the form of a comma-separated list as accepted by the launcher --bounce command (e.g. '00' or '00,01')"""
        cmdArr = Cluster.__LauncherCmdArr.copy()
        cmdArr.append("--down")
        cmdArr.append(nodes)
