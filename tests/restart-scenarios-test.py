...
# --kill-count <nodgst instances to kill>
...
try:
    ...
    Print("Stand up GST wallet kgstd")
    accountsCount=total_nodes
    walletName="MyWallet"
    Print("Creating wallet %s if one doesn't already exist." % walletName)
    wallet=walletMgr.create(walletName, [cluster.gstioAccount,cluster.defproduceraAccount,cluster.defproducerbAccount])
    ...
    gstioAccount=cluster.gstioAccount
    ...
    if not cluster.createAccounts(gstioAccount):
