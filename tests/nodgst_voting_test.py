...
try:
    ...
    for account in accounts:
        ...
        trans=node.createInitializeAccount(account, cluster.gstioAccount, stakedDeposit=0, waitForTransBlock=False, stakeNet=1000, stakeCPU=1000, buyRAM=1000, exitOnError=True)
        transferAmount="100000000.0000 {0}".format(CORE_SYMBOL)
        Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.gstioAccount.name, account.name))
        node.transferFunds(cluster.gstioAccount, account, transferAmount, "test transfer")
