...
try:
    ...
    specificExtraNodgstArgs[totalProducerNodes]="--plugin gstio::test_control_api_plugin"
    ...
    if cluster.launch(prodCount=prodCount, onlyBios=False, topo="bridge", pnodes=totalProducerNodes,
        ...
        Utils.errorExit("Failed to stand up gst cluster.")
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.gstioAccount,accounts[0],accounts[1],accounts[2],accounts[3],accounts[4]])
    ...
    for account in accounts:
        ...
        trans=node.createInitializeAccount(account, cluster.gstioAccount, stakedDeposit=0, waitForTransBlock=True, stakeNet=1000, stakeCPU=1000, buyRAM=1000, exitOnError=True)
        transferAmount="100000000.0000 {0}".format(CORE_SYMBOL)
        Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.gstioAccount.name, account.name))
        node.transferFunds(cluster.gstioAccount, account, transferAmount, "test transfer", waitForTransBlock=True)
