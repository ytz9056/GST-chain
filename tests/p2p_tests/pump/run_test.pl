...
my $gst_home = defined $ENV{GST_HOME} ? $ENV{GST_HOME} : getcwd;
my $gstd = $gst_home . "/programs/gstd/gstd";
my $gstc = $gst_home . "/programs/gstc/gstc";

my $nodes = defined $ENV{GST_TEST_RING} ? $ENV{GST_TEST_RING} : "1";
my $pnodes = defined $ENV{GST_TEST_PRODUCERS} ? $ENV{GST_TEST_PRODUCERS} : "1";

my $prods = 21;
my $genesis = "$gst_home/genesis.json";
...
if (!GetOptions("nodes=i" => \$nodes,
    ...
    print "where:\n";
    print "--nodes=n (default = 1) sets the number of gstd instances to launch\n";
...
sub write_config {
    ...
    if (defined $producer) {
        ...
        print $cfg "private-key = [\"GST6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV\",\"5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3\"]\n";

        print $cfg "plugin = gstio::producer_plugin\n";
        print $cfg "plugin = gstio::chain_api_plugin\n";
...
sub perform_work {
    ...
    if ($mode == 0) {
        ...
        my $counter = 0;
        while (time () < $stoptime) {
            `$gstc transfer gst inita 10 >> gstc.out 2>> gstc.err`;
            $counter++;
            if ($counter % 1000 == 0) {
