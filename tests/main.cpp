...
//extern uint32_t GST_TESTING_GENESIS_TIMESTAMP;
...
boost::unit_test::test_suite* init_unit_test_suite(int argc, char* argv[]) {
   ...
   /*
   const char* genesis_timestamp_str = getenv("GST_TESTING_GENESIS_TIMESTAMP");
   if( genesis_timestamp_str != nullptr )
   {
      GST_TESTING_GENESIS_TIMESTAMP = std::stoul( genesis_timestamp_str );
   }
   std::cout << "GST_TESTING_GENESIS_TIMESTAMP is " << GST_TESTING_GENESIS_TIMESTAMP << std::endl;
   */
   return nullptr;
}
