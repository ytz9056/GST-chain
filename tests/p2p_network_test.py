...
# gstio should have the same key as defproducera
gstio = copy.copy(defproduceraAccount)
gstio.name = "gstio"
...
wasmFile="gstio.system.wasm"
abiFile="gstio.system.abi"
Print("\nPush system contract %s %s" % (wasmFile, abiFile))
trans=node0.publishContract(gstio.name, wasmFile, abiFile, waitForTransBlock=True)
if trans is None:
    Utils.errorExit("Failed to publish gstio.system.")
else:
...
try:
    ...
    for cmdInd in range(maxIndex):
        (transIdList, checkacct, expBal, errmsg) = module.execute(cmdInd, node0, testeraAccount, gstio)
