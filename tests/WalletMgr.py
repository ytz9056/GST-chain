...
class WalletMgr(object):
    __walletLogOutFile="test_kgstd_out.log"
    __walletLogErrFile="test_kgstd_err.log"
    __walletDataDir="test_wallet_0"
    __MaxPort=9999

    # pylint: disable=too-many-arguments
    # walletd [True|False] True=Launch wallet(kgstd) process; False=Manage launch process externally.
    def __init__(self, walletd, nodgstPort=8888, nodgstHost="localhost", port=9899, host="localhost"):
        self.walletd=walletd
        self.nodgstPort=nodgstPort
        self.nodgstHost=nodgstHost
        self.port=port
        self.host=host
        self.wallets={}
    ...
    def getArgs(self):
        return " --url http://%s:%d%s %s" % (self.nodgstHost, self.nodgstPort, self.getWalletEndpointArgs(), Utils.MiscGstClientArgs)
    ...
    def launch(self):
        if not self.walletd:
            Utils.Print("ERROR: Wallet Manager wasn't configured to launch kgstd")
            return False
