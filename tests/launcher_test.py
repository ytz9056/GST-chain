...
try:
    ...
    if not dontLaunch:
        ...
        if walletMgr.launch() is False:
            ...
            errorExit("Failed to stand up gst walletd.")
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.gstioAccount,cluster.defproduceraAccount])
    ...
    Print("Create new account %s via %s" % (testeraAccount.name, cluster.gstioAccount.name))
    transId=node.createInitializeAccount(testeraAccount, cluster.gstioAccount, stakedDeposit=0, waitForTransBlock=False, exitOnError=True)

    Print("Create new account %s via %s" % (currencyAccount.name, cluster.gstioAccount.name))
    transId=node.createInitializeAccount(currencyAccount, cluster.gstioAccount, buyRAM=1000000, stakedDeposit=5000, exitOnError=True)

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.gstioAccount.name))
    transId=node.createInitializeAccount(exchangeAccount, cluster.gstioAccount, buyRAM=1000000, waitForTransBlock=True, exitOnError=True)
